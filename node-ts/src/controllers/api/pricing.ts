...
export const createItem = async (req: CreateItemRequest, res: Response): Promise<void> => {
  try {
    // Create new item using request body
    const newItem: IPricing = new PricingModel(req.body);
...
export const getAllItems = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const items = await PricingModel.find({ brancheId }).sort({ type: 1, createdAt: -1, activeState: -1 });
    res.status(201).json({
      success: true,
      errors: [],
      status: 200,
...
export const getAllItemsPagination = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const items = await PricingModel.find(filter)
      .skip((+page - 1) * +limit)
      .limit(+limit);

    // Count total number of items (for pagination)
    const totalCount = await PricingModel.countDocuments(filter);
...
export const getItemById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const item = await PricingModel.findById(req.params.id);
    if (!item) {
...
export const updateCategoryStopAllCategoresReletedToBill = async (req: Request, res: Response): Promise<void> => {
  debugger;
  try {
...
export const updateItem = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const updatedItem = await PricingModel.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!updatedItem) {
      res.status(404).json({ msg: 'Item not found' });
    }
    res.status(201).json({
...
export const deleteItem = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const deletedItem = await PricingModel.findByIdAndDelete(req.params.id);
    if (!deletedItem) {
      res.status(404).json({ msg: 'Item not found' });
    }
    res.status(201).json({
