...
class ClientController {
    constructor() {
        ...
        this.getAllItems = async (req, res) => {
            ...
            try {
                const items = await client_1.default.find({ brancheId }).sort({ createdAt: -1, activeState: 1 });
                res.status(201).json({
                    success: true,
                    errors: [],
                    status: 200,
        ...
        this.getItemById = async (req, res) => {
            try {
                const item = await client_1.default.findById(req.params.id);
                if (!item) {
        ...
        this.updateItem = async (req, res) => {
            try {
                const updatedItem = await client_1.default.findByIdAndUpdate(req.params.id, req.body, { new: true });
                if (!updatedItem) {
                    return res.status(404).json({ msg: 'Item not found' });
                }
                res.status(201).json({
        ...
        this.deleteItem = async (req, res) => {
            try {
                const deletedItem = await client_1.default.findByIdAndDelete(req.params.id);
                if (!deletedItem) {
                    return res.status(404).json({ msg: 'Item not found' });
                }
                res.status(201).json({
