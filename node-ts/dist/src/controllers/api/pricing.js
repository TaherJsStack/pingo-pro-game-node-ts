    ...
    constructor() {
        ...
        this.getAllItems = async (req, res) => {
            ...
            try {
                ...
                const items = await pricing_1.default.find({ brancheId }).sort({ type: 1, createdAt: -1, activeState: -1 });
                res.status(201).json({
                    success: true,
                    errors: [],
                    status: 200,
        ...
        this.getAllItemsPagination = async (req, res) => {
            try {
                ...
                const items = await pricing_1.default.find(filter)
                    .skip((+page - 1) * +limit)
                    .limit(+limit);
                // Count total number of items (for pagination)
                const totalCount = await pricing_1.default.countDocuments(filter);
                res.status(200).json({
                    success: true,
                    data: {
                        items,
        ...
        this.getItemById = async (req, res) => {
            try {
                ...
                const item = await pricing_1.default.findById(req.params.id);
                if (!item) {
        ...
        this.updateItem = async (req, res) => {
            try {
                ...
                const updatedItem = await pricing_1.default.findByIdAndUpdate(req.params.id, req.body, { new: true });
                if (!updatedItem) {
                    res.status(404).json({ msg: 'Item not found' });
                }
                res.status(201).json({
        ...
        this.deleteItem = async (req, res) => {
            try {
                ...
                const deletedItem = await pricing_1.default.findByIdAndDelete(req.params.id);
                if (!deletedItem) {
                    res.status(404).json({ msg: 'Item not found' });
                }
                res.status(201).json({
