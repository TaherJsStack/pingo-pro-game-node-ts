...
exports.createItem = async (req, res) => {
  ...
  try {
    ...
    let newItem = new PricingModel(req.body);
...
exports.updateCategoryStopAllCategoresReletedToBill = async (req, res) => {
  debugger
  try {
    ...
    let objectIds = await ids.map(id => new ObjectId(id));
    ...
    if (updatedItems.nModified === 0) {
      return res.status(404).json({ msg: 'No categories updated' });
    }
...
exports.updateItem = async (req, res) => {
  try {
    ...
    const updatedItem = await PricingModel.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
