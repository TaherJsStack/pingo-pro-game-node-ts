...
(async () => {
  ...
  subscription = await InvoiceService.getDataObservable().subscribe( async data => {
...
async function deletedListSession(data) {
  ...
  for (let index = 0; index < data.idsToDelete.length; index++) {
    ...
    let existingInvoice = await InvoiceModel.findOne({ sessionId: data.idsToDelete[0] });
...
exports.updateLockBill = async (req, res) => {
...
// Update - PUT request handler
exports.updateItemMenuItems = async (req, res) => {
